...
def _owned_paths():
    return (
        [("delta_location", i) for i in range(3)]
        + [("delta_rotation_euler", i) for i in range(3)]
        + [("delta_scale", i) for i in range(3)]
    )
...
def apply_object(obj, context):
    ...
    for i in range(3):
        ...
        expressions.append(("delta_location", i, f"{base_loc[i]:.10f} + {slow}{bob_expr}{jitter}"))

    for i in range(3):
        expression = f"{base_rot[i]:.10f} + sin(frame*{(0.018 + i * 0.004) * speed:.10f}+{phases[6 + i]:.10f})*{wobble:.10f}"
        expressions.append(("delta_rotation_euler", i, expression))

    for i in range(3):
        expression = f"{base_scale[i]:.10f} + sin(frame*{0.035 * speed:.10f}+{phases[(i + 2) % 9]:.10f})*{scale_amt:.10f}"
        expressions.append(("delta_scale", i, expression))

    for path, index, expression in expressions:
        if utils.add_owned_driver(obj, path, index, expression, MARKER) is None:
            clear_object(obj, restore=True)
            return False, f"could not create DSM driver for {path}"
